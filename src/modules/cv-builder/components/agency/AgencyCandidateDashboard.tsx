  ...
  return (
    <div className="min-h-screen bg-[var(--color-neutral-text-primary)] text-white p-6 md:p-10 font-sans">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-white/10 pb-4 gap-4">
          <div className="flex items-center gap-3">
            <button 
              ...
              className={`p-2 rounded-[${radius.card}] bg-white/10 hover:bg-white/20 text-white transition flex items-center gap-2 text-xs font-bold cursor-pointer`}
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3.5 py-2 bg-[var(--color-accent-purple)] hover:opacity-90 text-white text-xs font-extrabold rounded-[${radius.card}] transition flex items-center gap-2 ${elevationSystem.floating} cursor-pointer`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="relative md:col-span-2">
            ...
            <input 
              ...
              className={`w-full bg-black/40 border border-white/20 rounded-[${radius.card}] pl-9 pr-4 py-2.5 text-xs text-white placeholder-white/40 outline-none focus:border-[var(--color-accent-purple)] transition`}
            />
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select 
              ...
              className={`w-full bg-black/40 border border-white/20 rounded-[${radius.card}] px-3 py-2.5 text-xs text-white outline-none focus:border-[var(--color-accent-purple)] transition cursor-pointer`}
            >
        ...
        {/* Candidates Table */}
        <div className={`bg-black/40 border border-white/10 rounded-[${radius.modal}] overflow-hidden ${elevationSystem.overlay}`}>
            ...
            <tbody className="divide-y divide-white/10">
              {filteredCandidates.map(candidat => (
                <tr key={candidat.id} className="hover:bg-white/5 transition">
                  ...
                  <td className="p-4"><span className={`px-2.5 py-1 rounded-[${radius.control}] bg-white/10 text-white font-medium`}>{candidat.vacant}</span></td>
